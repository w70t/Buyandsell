...
Working: Web UI + admin panel use a modern inline-SVG icon set (app/web/icons.py); all 34 backend tests pass. Flutter app gained modern micro-interactions: haptics (fav/tabs/categories/contact), press-scale (cards/categories/search), staggered grid entrance, and Hero image transition card→detail (new PressableScale/EntranceFade in widgets/common.dart). The bottom nav bar is a floating pill glass bar (rounded + shadow + BackdropFilter, extendBody) with a sliding frosted indicator that animates to the active tab (AnimatedPositionedDirectional, RTL-aware; hidden under the center sell FAB) — iOS-style. The listing-detail action bar is also a frosted glass bar; tab screens get glassNavInset() bottom padding so content clears the floating bar. All top app bars are subtle glass bars (reusable GlassBar/glassAppBar in widgets/common.dart): home SliverAppBar, plus favorites/conversations/profile/post-ad via extendBodyBehindAppBar + glassTopInset() top padding — content blurs behind the bar on scroll. Loading skeleton shimmer (widgets/skeleton.dart) reworked: diagonal, soft 5-stop highlight, continuous no-dead-time sweep. Grid skeleton wraps the whole grid in ONE shimmer (SliverToBoxAdapter + shrinkWrap GridView) so the highlight travels across the entire grid diagonally instead of per-card.
In progress: none
